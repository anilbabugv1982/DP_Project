...
public class TestNGCustomReportListener extends GetDriverInstance implements IReporter {

	WebDriver labelDriverInstance;
 ...
 // ~ Methods --------------------------------------------------------------
    ...
    private void resultSummary(ISuite suite, IResultMap tests, String testname,
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                if(style.equalsIgnoreCase("failed")) {
                	buff.append("<td bgcolor=\"red\"><a href=\"#m"
                            + methodIndex
                            + "\">"
                            ...
                            + testInstanceName + ")") + "</td>"
                            + "<td bgcolor=\"red\" class=\".stripe\" style=\"text-align:left;padding-right:2em\">" + firstLine+"<br/>"+screenshotLnk+ "</td>"
                            + "<td style=\"text-align:right\">" + formatter.format(calendar.getTime()) + "</td>" + "<td class=\"numi\">"
                            + millisToTimeConversion(end - start) + "</td>" + "</tr>");
                }else if(style.equalsIgnoreCase("passed")) {
                	buff.append("<td bgcolor=\"#00FF00\"><a href=\"#m"
                            + methodIndex
                            + "\">"
                            + qualifiedName(method)
                            + " "
                            ...
                            + (null == testInstanceName ? "" : "<br>("
                            + testInstanceName + ")") + "</td>"
                            + "<td bgcolor=\"#00FF00\" class=\".stripe\" style=\"text-align:left;padding-right:2em\">" + firstLine+"<br/>"+screenshotLnk+ "</td>"
                            + "<td style=\"text-align:right\">" + formatter.format(calendar.getTime()) + "</td>" + "<td class=\"numi\">"
                            + millisToTimeConversion(end - start) + "</td>" + "</tr>");
                }else if(style.equalsIgnoreCase("skipped")) {
                	buff.append("<td bgcolor=\"grey\"><a href=\"#m"
                            + methodIndex
                            + "\">"
                            + qualifiedName(method)
                            + " "
                            ...
                            + (null == testInstanceName ? "" : "<br>("
                            + testInstanceName + ")") + "</td>"
                            + "<td bgcolor=\"grey\" class=\".stripe\" style=\"text-align:left;padding-right:2em\">" + firstLine+"<br/>"+screenshotLnk+ "</td>"
                            + "<td style=\"text-align:right\">" + formatter.format(calendar.getTime()) + "</td>" + "<td class=\"numi\">"
                            + millisToTimeConversion(end - start) + "</td>" + "</tr>");
                }
                //resultSet.size()
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites) {
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r : tests.values()) {
                ...
                qty_pass_m += q;
                summaryCell(q, testType, Integer.MAX_VALUE);
				/*q = overview.getPassedTests().size();
				qty_pass_s += q;
				summaryCell(q, Integer.MAX_VALUE);*/
                q = getMethodSet(overview.getFailedTests(), suite).size();
                qty_fail += q;
                testType = "FailedTests";
                summaryCell(q, testType, 0);
                ...
                summaryCell(summaryFormat.format(overview.getStartDate()),testType,true);
                out.println("</td>");

                summaryCell(summaryFormat.format(overview.getEndDate()),testType,true);
                out.println("</td>");
                ...
                summaryCell(millisToTimeConversion((overview.getEndDate().getTime() - overview
                        .getStartDate().getTime()) / 1000), testType,true);
        ...
        if (qty_tests > 1) {
            ...
            summaryCell(qty_pass_m, testType,Integer.MAX_VALUE);
            //summaryCell(qty_pass_s, Integer.MAX_VALUE);
            summaryCell(qty_fail, testType, 0);
            summaryCell(qty_skip, testType, 0);
           
            //summaryCell(" ", true);
            summaryCell(" ", testType, true);
            summaryCell(" ", testType, true);
            //summaryCell(" ", true);
			/*summaryCell(
					formatter.format(((time_end - time_start) / 1000.) / 60.)
					+ " minutes", true);*/
            summaryCell(millisToTimeConversion(((time_end - time_start) / 1000)), testType, true);
    ...
    private void summaryCell(String[] val) {
        ...
        summaryCell(b.toString(), testType, true);
    }

    private void summaryCell(String v, String testType, boolean isgood) {
    	if(testType.equalsIgnoreCase("FailedTests"))
    		out.print("<td bgcolor=\"red\" class=\"numi" + (isgood ? "" : "_attn") + "\">" + v
                + "</td>");
    	else
    ...
    private void summaryCell(int v, String testType,int maxexpected) {
        summaryCell(String.valueOf(v), testType, v <= maxexpected);
    }
    ...
    protected void startHtml(PrintWriter out) {
        ...
        out.println("table {margin-bottom:30px;border-collapse:collapse;empty-cells:show}");
